...
class Marker extends Module_Base {
	...
	public function get_keywords() {
		return [ 'marker', 'pointer', 'hotspots', 'spot', 'hot', 'image hotspot', 'image pointer', 'tooltip' ];
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'marker_always_visible_tooltip',
			[ 
				'label'       => esc_html__( 'Always Visible Tooltip', 'bdthemes-element-pack' ) . BDTEP_NC,
				'description' => esc_html__( 'Always show tooltips, even without interaction.', 'bdthemes-element-pack' ),
				'type'        => Controls_Manager::SWITCHER,
			]
		);
	...
	protected function render() {
		...
		if ( 'yes' === $settings['marker_animation'] ) {
			...
			// Only apply scrollspy animation if always-visible tooltips are not enabled
			// to prevent conflicts with tooltip positioning
			$has_always_visible_tooltips = false;
			foreach ( $settings['markers'] as $marker ) {
				if ( $marker['marker_title'] && $marker['marker_tooltip'] && 'yes' === $settings['marker_always_visible_tooltip'] ) {
					$has_always_visible_tooltips = true;
					break;
				}
			}
			
			if ( ! $has_always_visible_tooltips ) {
			...
			<div <?php $this->print_render_attribute_string( 'wrapper' ); ?>>
				...
				foreach ( $settings['markers'] as $index => $marker ) {
					...
					if ( $marker['marker_title'] and $marker['marker_tooltip'] ) {
						...
						if ( 'yes' == $settings['marker_always_visible_tooltip'] ) {
							$this->add_render_attribute( $element_key, 'data-always-show', 'true', true );
						}

						if ( 'yes' == $settings['marker_tooltip_trigger'] ) {
							$this->add_render_attribute( $element_key, 'data-tippy-trigger', 'click', true );
						} else {
							$this->add_render_attribute( $element_key, 'data-tippy-trigger', 'mouseenter focus', true );
						}
					}
