...
class Table extends Module_Base {
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[ 
				...
				'options' => [ 
					...
					'repeater'     => __( 'Meta Fields (ACF)', 'bdthemes-element-pack' ) . BDTEP_NC,
				],
			]
		);
		...
		// Meta Fields/Repeater Settings Start
		$this->start_controls_section(
			'section_meta_fields_settings',
			[ 
				'label'     => __( 'Meta Fields Settings', 'bdthemes-element-pack' ) . BDTEP_NC,
				'tab'       => Controls_Manager::TAB_CONTENT,
				'condition' => [ 
					'source' => 'repeater',
				],
		...
		$this->add_control(
			'meta_post_id',
			[ 
				'label'       => __( 'Post ID', 'bdthemes-element-pack' ),
				'type'        => Controls_Manager::TEXT,
				'default'     => '',
				'placeholder' => __( 'Leave empty for current post', 'bdthemes-element-pack' ),
				'description' => __( 'Enter a specific post ID or leave empty to use the current post.', 'bdthemes-element-pack' ),
				'label_block' => true,
				'dynamic'     => [ 'active' => true ],
				'separator'   => 'before',
			]
		...
		$this->add_control(
			...
			[ 
				'label'       => __( 'Repeater Field Name', 'bdthemes-element-pack' ),
				'dynamic' => ['active' => false],
                'type'    => Dynamic_Select::TYPE,
				'default'     => '',
				'placeholder' => __( 'Enter repeater field name', 'bdthemes-element-pack' ),
				'description' => __( 'Enter the field name of your ACF repeater field.', 'bdthemes-element-pack' ),
				'label_block' => true,
				'query_args'  => [
                    'query'        => 'acf',
                    'field_type' => ['repeater'],
		...
		$meta_head_repeater = new Repeater();

		$meta_head_repeater->add_control(
			'meta_field_name',
			[ 
				'label'       => __( 'Field Name', 'bdthemes-element-pack' ),
				'type'        => Dynamic_Select::TYPE,
				'default'     => '',
				'placeholder' => __( 'Enter sub-field name', 'bdthemes-element-pack' ),
				'description' => __( 'Enter the sub-field name from your repeater (e.g., name, age, phone).', 'bdthemes-element-pack' ),
				'label_block' => true,
				'query_args'  => [
                    'query'        => 'acf',
                    'field_type'   => ['text', 'textarea', 'wysiwyg', 'number', 'email', 'url', 'password', 'range', 'date_picker', 'date_time_picker', 'time_picker', 'color_picker', 'select', 'checkbox', 'radio', 'true_false', 'link', 'post_object', 'page_link', 'relationship', 'taxonomy', 'user', 'image', 'file', 'oembed', 'gallery', 'group'],
                ],
			]
		);
		...
		$this->add_control(
			...
			[ 
				...
				'fields'      => $meta_head_repeater->get_controls(),
				'default'     => [ 
					[ 
				...
				'title_field' => '{{{ meta_field_name }}}',
			]
		);

		$this->end_controls_section();
		// Meta Fields/Repeater Settings End
	...
	/**
	 * Render Meta Fields/Repeater Table
	 */
	public function meta_fields_table() {
		$settings = $this->get_settings_for_display();

		// Get post ID
		if ( ! empty( $settings['meta_post_id'] ) ) {
			$post_id = $settings['meta_post_id'];

		} else {
			// Elementor Free — fix needed
			$post_id = get_queried_object_id() ?: get_the_ID();
		}
		
		// Get repeater field name
		$repeater_field = $settings['meta_repeater_field_name'];
		
		if ( empty( $repeater_field ) ) {
			echo '<div data-bdt-alert class="bdt-alert-warning">
					<a class="bdt-alert-close" data-bdt-close></a>
					<p>' . esc_html__( 'Please enter a repeater field name.', 'bdthemes-element-pack' ) . '</p>
				  </div>';
			return;
		}

		// Get repeater data - supports ACF
		$repeater_data = [];
		...
		// Fallback to standard get_post_meta
			...
			$repeater_data = get_post_meta( $post_id, $repeater_field, true );
		}

		if ( empty( $repeater_data ) || ! is_array( $repeater_data ) ) {
			echo '<div data-bdt-alert class="bdt-alert-warning">
					<a class="bdt-alert-close" data-bdt-close></a>
					<p><strong>' . esc_html__( 'No data found for this repeater field.', 'bdthemes-element-pack' ) . '</strong></p>
				  </div>';
			return;
		}

		$columns = $settings['meta_table_columns'];

		?>

		<table class="bdt-meta-fields-table">
			<thead>
				<tr>
					<?php 
					$acf_global = new ACF_Global();
					foreach ( $columns as $column ) :
						$label = $acf_global->get_acf_subfield_label( $repeater_field, $column['meta_field_name'], $post_id );
						?><th><?php echo esc_html( $label ); ?></th>
					<?php endforeach; ?>
				</tr>
			</thead>
			<tbody>
				<?php foreach ( $repeater_data as $row_index => $row_data ) : ?>
					<tr>
						<?php foreach ( $columns as $column ) : 
							$field_name = $column['meta_field_name'];
							$field_value = '';
							...
							// Get field object for formatting
							$field_object = $acf_global->get_acf_subfield_object( $repeater_field, $field_name, $post_id );

							?>
							<td>
								<?php 
								// Format field value using ACF_Global
								echo $acf_global->format_acf_field_value( $field_value, $field_object );
								?>
							</td>
						<?php endforeach; ?>
					</tr>
	...
	protected function render() {
		...
		<div <?php $this->print_render_attribute_string( 'table-wrapper' ); ?>>
			...
			if ( 'custom' == $settings['source'] ) {
				...
				$this->static_table();
			} elseif ( 'repeater' == $settings['source'] ) {
				$this->meta_fields_table();
