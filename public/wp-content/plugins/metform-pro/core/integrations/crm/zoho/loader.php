...
class Integration
{
    ...
    public function contents()
    {
       ...
       $disconnect_icon = '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 14 14" fill="none">
        ...
        if( method_exists(Util::class, 'is_top_tier') && !Util::is_top_tier() && !Util::is_using_settings_option('mf_zoho_data_center') && !Util::is_using_feature('mf_zoho') && !Util::is_old_pro_user() ){

            $alert_heading = esc_html__('Upgrade to integrate Zoho with your forms!', 'metform-pro');
            $alert_description = esc_html__('Get premium access to integrate Zoho with your forms.', 'metform-pro');

            if( !\MetForm\Utils\Util::is_mid_tier() && !\MetForm\Utils\Util::is_top_tier()  ){
                $alert_heading = esc_html__('Zoho Integration is Exclusive To Higher Tiers!', 'metform-pro');
                $alert_description = esc_html__('Get access by upgrading to MetForm Agency Plan.', 'metform-pro');
            }

            Util::mf_pro_alert_notice([
